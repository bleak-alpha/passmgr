...
dictionary = numpy.concatenate([num, lowcase, upcase, symbol])
...
rand_lower = random.choice(lowcase)
rand_symbol = random.choice(symbol)
 
# combine the character randomly selected above at this stage, the password contains only 4 characters but
...
for x in range(pass_len - 4):
    infant_pass = infant_pass + random.choice(jumbled_mess)
    ...
    random.shuffle(infant_pass_list)
